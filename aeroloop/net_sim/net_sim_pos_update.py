...
#very simple implementation just for demostration 
#Index 0 is the UAV1 and 1 the edge 
def pos_update(nodes,socket):

    print("position updater started")
    while True:

        idx = 1
        print("waiting for possition messages")
        topic, data = socket.recv_multipart()
        d=json.loads(data.decode('utf-8'))

        pos=from_dict(data_class=NSPos,data=d)


        if topic.decode("utf-8") == "UAV1":
            idx = 0

        node =nodes.Get(idx)
        mm = node.GetObject( ns.mobility.MobilityModel.GetTypeId() )
        # Set postion coordinates in the ns-3 node
        print(f"set position update for {idx} node")
        pos = mm.SetPosition( ns.core.Vector( pos.x, pos.y, pos.z ) )
...
def main(argv):
    ...
    tapBridge.SetAttribute ("DeviceName", ns.core.StringValue ("wu1U"));
    ...
    # ghost node.
    #
    tapBridge.SetAttribute ("DeviceName", ns.core.StringValue ("we1U"));
    tapBridge.Install (nodes.Get (1), devices.Get (1));

    #
    # Initialize the pub/sub and start the position update thread
    #

    context = zmq.Context()
    socket  = context.socket(zmq.SUB)
    #uav
    socket.connect("tcp://192.168.122.125:5556")
    #edge
    socket.connect("tcp://192.168.122.126:5556")
    socket.subscribe("")

    pos_thread = threading.Thread(target=pos_update, args=(nodes,socket,))
